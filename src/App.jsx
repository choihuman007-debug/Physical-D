...
export default function App(){
  ...
  const [athletes,setAthletes]=useState([
    {id:"PD0001",name:"김민수",sport:"야구",position:"투수",team:"",height:"183",weight:"85",age:"26",memo:"우측 어깨 가벼운 불편감. 어깨 안정성 강화 목표.",equipment:["Barbell","Dumbbell","Cable","Band","MedBall","Bodyweight","Box","Machine"]},
    {id:"PD0002",name:"이준혁",sport:"야구",position:"외야수",team:"",height:"178",weight:"80",age:"24",memo:"타격 파워, 스프린트 속도 개선 목표. 좌측 햄스트링 긴장 이력.",equipment:["Barbell","Dumbbell","Cable","Band","MedBall","Bodyweight","TrapBar","Machine"]},
  ]);
  const [cfg,setCfg]=useState({athleteId:"PD0001",conceptId:"lower-power",weekNum:1,date:new Date().toISOString().split("T")[0]});
  ...
  const saveSession=()=>{if(!session||!athlete)return;setHistory(h=>[{id:`S${Date.now()}`,athleteId:athlete.id,athleteName:athlete.name,athleteSport:athlete.sport,athletePosition:athlete.position,concept:concept.label,weekNum:cfg.weekNum,date:cfg.date,session:JSON.parse(JSON.stringify(session)),feedback:null},...h]);setSession(null);setKakao("");};
  const saveFeedback=(hid)=>{setHistory(h=>h.map(x=>x.id===hid?{...x,feedback:{...feedback}}:x));setFeedbackFor(null);setFeedback({q1:5,q2:5,q3:5,q4:3,q5area:"",q5vas:0});};
  const saveAthlete=(a)=>{const final=a.id?a:{...a,id:nextAthleteId(athletes)};setAthletes(p=>{const i=p.findIndex(x=>x.id===final.id);if(i>=0){const n=[...p];n[i]=final;return n;}return[...p,final];});setEditAthlete(null);};
      ...
      {session&&(
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:16,marginTop:16}}>
          <div style={{...s.card,padding:16}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:16,flexWrap:"wrap"}}><button style={s.btn("s")} onClick={saveSession}>✅ 승인 & 저장</button><button style={s.btn("o")} onClick={generate}>🔄 재생성</button></div>
          </div>
          <div style={{...s.card,padding:16}}>
      ...
      {athletes.map(a=>(
        <div key={a.id} style={s.card}>
          <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
            ...
            <div style={{display:"flex",gap:8}}><span onClick={()=>setEditAthlete(a)} style={{cursor:"pointer",fontSize:12,color:C.brandLight,fontWeight:600}}>수정</span><span onClick={()=>{if(athletes.length>1){setAthletes(p=>p.filter(x=>x.id!==a.id));if(cfg.athleteId===a.id)setCfg(c=>({...c,athleteId:athletes.find(x=>x.id!==a.id)?.id||""}))}}} style={{cursor:"pointer",fontSize:12,color:C.danger}}>삭제</span></div>
          </div>
      ...
      <div style={{display:"flex"}}>
        ...
        {!isMobile&&(
          <div style={{width:sideW,background:C.brand,display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh"}}>
            ...
            <div style={{padding:"12px 18px",borderTop:"1px solid rgba(255,255,255,0.08)",fontSize:10,color:"rgba(255,255,255,0.3)"}}>{LIBRARY.length}개 운동 · {athletes.length}명</div>
          </div>
        )}
        {/* Main content */}
